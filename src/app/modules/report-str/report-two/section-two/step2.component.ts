...
export class Step2Component implements OnInit, OnDestroy {
  ...
  showTable: boolean = false; // Biến điều khiển hiển thị table
  ...
  createLegalRepresentative(item?: Number) {
    const modalRef = this.modalService.open(AddLegalRepresentativeComponent, { size: 'xl' });
  ...
  deleteOrganization(id: number | undefined) {
    const isDelete = confirm('Bạn có chắc chắn muốn xóa tổ chức báo cáo này?');
    if (isDelete) {
  ...
  toggleTable(event: Event): void {
    ...
    this.showTable = checkbox.checked;
  }
