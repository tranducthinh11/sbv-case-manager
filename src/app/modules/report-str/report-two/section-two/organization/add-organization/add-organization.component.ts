...
export class AddOrganizationComponent extends BaseDateRangeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.organizationService.updateOrganizationValidators(this.addFormOrganization, this.addFormOrganization.get('khach_hang_hien_huu')?.value, this.validatorsArray);
  }
  ...
  initFormOrganization(): void {
    this.addFormOrganization = this.fb.group({
      ...
      ten_to_chuc: ['', Validators.required],
      ten_tieng_nuoc_ngoai: [''],
      ten_viet_tat: [''],
      loai_hinh_to_chuc: this.fb.group({
        ma_loai_hinh: ['1', Validators.required],
        mo_ta: ['']
      }),
      ...
      giay_phep_thanh_lap: this.fb.group({
        so_giay_phep: [''],
        ngay_cap: [null as NgbDateStruct | null],
        noi_cap: ['']
      }),
      ma_so_doanh_nghiep: this.fb.group({
        ma_so: ['', [Validators.required, ValidateHelper.mstValidator]],
        ngay_cap: [null as NgbDateStruct | null],
        noi_cap: ['']
      }),
      nganh_nghe_kinh_doanh: ['', Validators.required],
  ...
  storeOriginalOrganizationValidators(): void {
    this.originalOrgValidators['ten_to_chuc'] = this.addFormOrganization.get('ten_to_chuc')?.validator;
    this.originalOrgValidators['so_dien_thoai'] = this.addFormOrganization.get('so_dien_thoai')?.validator;
    this.originalOrgValidators['nganh_nghe_kinh_doanh'] = this.addFormOrganization.get('nganh_nghe_kinh_doanh')?.validator;

    const loaiHinhGroup = this.addFormOrganization.get('loai_hinh_to_chuc') as FormGroup;
    if (loaiHinhGroup) {
      this.originalOrgLoaiHinhToChucValidators['ma_loai_hinh'] = loaiHinhGroup.get('ma_loai_hinh')?.validator;
    }


    const diaChiGroup = this.addFormOrganization.get('dia_chi') as FormGroup;
    if (diaChiGroup) {
    ...
    const msdnGroup = this.addFormOrganization.get('ma_so_doanh_nghiep') as FormGroup;
    if (msdnGroup) {
  ...
  listenToKhachHangHienHuuOrgChanges(): void {
    const control = this.addFormOrganization.get('khach_hang_hien_huu');
    if (control) {
      this.khachHangHienHuuOrgSub = control.valueChanges.subscribe(value => {
        this.organizationService.updateOrganizationValidators(this.addFormOrganization, value, this.validatorsArray);
      });
    }
  }
  ...
  updateTypeOrganization(event: Event) {
    ...
    if (selectedJob) {
      this.addFormOrganization.get('loai_hinh_to_chuc.mo_ta')?.setValue(selectedJob.name);
    }
  }

  addOrganizationSubmit() {
    if (confirm('Bạn có chắc chắn muốn thêm tổ chức thực hiện giao dịch này?')) {

      if (this.addFormOrganization.invalid) {
        this.addFormOrganization.markAllAsTouched();
        alert("Vui lòng nhập đầy đủ thông tin bắt buộc.");
      } else {
        const khachHangHienHuu = this.addFormOrganization.get('khach_hang_hien_huu')?.value;
        const taiKhoan = (this.addFormOrganization.get('tai_khoan') as FormArray).length > 0 && (this.addFormOrganization.get('tai_khoan') as FormArray).valid;
        const maSoDoanhNghiep = this.addFormOrganization.get('ma_so_doanh_nghiep.ma_so')?.value.length > 0;
