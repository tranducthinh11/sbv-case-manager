...
export class AddOrganizationInformationAccountComponent extends BaseDateRangeComponent implements OnInit {
  ...
  addOrganizationSubmit(): void {
    ...
    if (this.addOrganizationAccountForm.invalid) {
      this.addOrganizationAccountForm.markAllAsTouched(); // Đánh dấu tất cả field để hiển thị lỗi
      return;
    }

    console.log("Submit thành công", this.addOrganizationAccountForm.value);
  ...
  onReset(): void {
    if (confirm('Bạn có chắc chắn muốn xóa trắng biểu mẫu không?')) {
      this.addOrganizationAccountForm.reset();
    }
  }
}
